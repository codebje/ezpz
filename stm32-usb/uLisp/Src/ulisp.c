/* uLisp ARM Version 3.4 - www.ulisp.com
   ...
   Modified for eZ-pZ, based on version 3.4. Original from:

   https://github.com/technoblogy/ulisp-arm/tree/0f8cca9009720cd0420fbf553243bd13d0e7794a
...
#define lineeditor
#define vt100
#define consolesupport		// stream via putc/getch instead of serial
// #define spisupport
// #define i2csupport
...
enum function {
	NIL, TEE, NOTHING, OPTIONAL, INITIALELEMENT, ELEMENTTYPE, BIT, AMPREST, LAMBDA, LET,
	LETSTAR, CLOSURE,
SPECIAL_FORMS,
	QUOTE, DEFUN, DEFVAR, SETQ, LOOP, RETURN, PUSH, POP, INCF, DECF, SETF, DOLIST, DOTIMES,
	TRACE, UNTRACE, FORMILLIS, DEFCODE,
TAIL_FORMS,
	PROGN, IF, COND, WHEN, UNLESS, CASE, AND, OR,
FUNCTIONS,
	NOT, NULLFN, CONS, ATOM, LISTP, CONSP, SYMBOLP, ARRAYP, BOUNDP, SETFN, STREAMP, EQ,
	CAR, FIRST, CDR, REST, CAAR, CADR, SECOND, CDAR, CDDR, CAAAR, CAADR, CADAR, CADDR,
	THIRD, CDAAR, CDADR, CDDAR, CDDDR, LENGTH, ARRAYDIMENSIONS, LIST, MAKEARRAY, REVERSE,
	NTH, AREF, ASSOC, MEMBER, APPLY, FUNCALL, APPEND, MAPC, MAPCAR, MAPCAN, ADD, SUBTRACT,
	MULTIPLY, DIVIDE, MOD, ONEPLUS, ONEMINUS, ABS, RANDOM, MAXFN, MINFN, NOTEQ, NUMEQ,
	LESS, LESSEQ, GREATER, GREATEREQ, PLUSP, MINUSP, ZEROP, ODDP, EVENP, INTEGERP, NUMBERP,
	FLOATFN, FLOATP, SIN, COS, TAN, ASIN, ACOS, ATAN, SINH, COSH, TANH, EXP, SQRT, LOG,
	EXPT, CEILING, FLOOR, TRUNCATE, ROUND, CHAR, CHARCODE, CODECHAR, CHARACTERP, STRINGP,
	STRINGEQ, STRINGLESS, STRINGGREATER, SORT, STRINGFN, CONCATENATE, SUBSEQ,
	READFROMSTRING, PRINCTOSTRING, PRIN1TOSTRING, LOGAND, LOGIOR, LOGXOR, LOGNOT, ASH,
	LOGBITP, EVAL, GLOBALS, LOCALS, MAKUNBOUND, BREAK, READ, PRIN1, PRINT, PRINC, TERPRI,
	READBYTE, READLINE, WRITEBYTE, WRITESTRING, WRITELINE, GC, DELAY, MILLIS,
	SLEEP, PPRINT, PPRINTALL, FORMAT,
KEYWORDS, 
#if defined(CPU_L073)
...
#endif
USERFUNCTIONS,
	ZDI_READ, ZDI_WRITE, ZDI_IN, ZDI_OUT, ZDI_STATUS, ZDI_REG, ZDI_STACK, ZDI_EXEC, ZDI_STOP,
	ZDI_RUN, ZDI_BRK, ZDI_STEP, ZDI_OVER, ZDI_UNTIL, ZDI_MEM, HEXLOAD,
ENDFUNCTIONS };
...
void pinthex(uint32_t, pfun_t);
void testescape();
char *lookupbuiltin (symbol_t name);
char *symbolname(symbol_t);
char *lookupsymbol(symbol_t);
uint8_t getminmax(symbol_t);
void checkminmax (symbol_t, int);
...
void errorsub (symbol_t fname, PGM_P string) {
  pfl(pserial); pstring(PSTR("Error: "), pserial);
  if (fname) {
    pserial('\'');
    pstring(symbolname(fname), pserial);
    pserial('\''); pserial(' ');
  }
  pstring(string, pserial);
}
...
const char badstream[] PROGMEM = "unsupported stream type";
...
#elif !defined(CPU_NRF51822) && !defined(CPU_NRF52833) && !defined(ARDUINO_FEATHER_F405) && !defined(CPU_L073)
inline int serial1read () { while (!Serial1.available()) testescape(); return Serial1.read(); }
#endif
#if defined(sdcardsupport)
...
#endif
    default:
    	error2(0, PSTR("unknown stream type"));
    	break;
  }
  return gfun;
}
...
// XXX should this also test !defined(ARDUINO_FEATHER_F405)?
#elif !defined(CPU_NRF51822) && !defined(CPU_NRF52833) && !defined(CPU_L073)
inline void serial1write (char c) { Serial1.write(c); }
#endif
#if defined(sdcardsupport)
...
#define ppspecials 18
const char ppspecial[ppspecials] PROGMEM = 
  { DOTIMES, DOLIST, IF, SETQ, TEE, LET, LETSTAR, LAMBDA, WHEN, UNLESS, FORMILLIS };
...
void supersub (object *form, int lm, int super, pfun_t pfun) {
  ...
  while (form != NULL) {
    if (atom(form)) { pstring(PSTR(" . "), pfun); printobject(form, pfun); pfun(')'); return; }
...
object *divide_floats (object *args, float fresult) {
  while (args != NULL) {
    ...
    if (f == 0.0) error2(DIVIDE, PSTR("division by zero")); // @suppress("Direct float comparison")
    fresult = fresult / f;
    args = cdr(args);
  }
  return makefloat(fresult);
...
object *fn_mod (object *args, object *env) {
  ...
  if (integerp(arg1) && integerp(arg2)) {
    ...
    if (fdivisor == 0.0) error2(MOD, PSTR("division by zero")); // @suppress("Direct float comparison")
...
object *fn_noteq (object *args, object *env) {
  ...
  while (args != NULL) {
    ...
    while (nargs != NULL) {
      ...
      } else if ((checkintfloat(NOTEQ, arg1) == checkintfloat(NOTEQ, arg2))) return nil; // @suppress("Direct float comparison")
      nargs = cdr(nargs);
    }
    args = cdr(args);
  }
...
object *fn_numeq (object *args, object *env) {
  ...
  while (args != NULL) {
    ...
    if (integerp(arg1) && integerp(arg2)) {
      if (!((arg1->integer) == (arg2->integer))) return nil;
    } else if (!(checkintfloat(NUMEQ, arg1) == checkintfloat(NUMEQ, arg2))) return nil; // @suppress("Direct float comparison")
    arg1 = arg2;
    args = cdr(args);
  }
  return tee;
...
object *fn_zerop (object *args, object *env) {
  (void) env;
  object *arg = first(args);
  if (floatp(arg)) return ((arg->single_float) == 0.0) ? tee : nil; // @suppress("Direct float comparison")
  else if (integerp(arg)) return ((arg->integer) == 0) ? tee : nil;
...
object *fn_gc (object *obj, object *env) {
  ...
  pstring(PSTR("Space: "), pserial);
  pint(Freespace - initial, pserial);
  pstring(PSTR(" bytes, Time: "), pserial);
...
object *fn_zdi_read (object *args, object *env) {
  (void) env;
  int reg = checkinteger(ZDI_READ, first(args));
  return number(zdi_read(reg));
}

object *fn_zdi_write(object *args, object *env) {
  int reg = checkinteger(ZDI_WRITE, first(args));
  int val = checkinteger(ZDI_WRITE, second(args));
  zdi_write(reg, val);
  return nil;
}

object *fn_ihex_load(object *args, object *env)
{
	(void)args;
	(void)env;
	ihex_load();
...
object *fn_in(object *args, object *env)
{
	int port = checkinteger(ZDI_IN, first(args));
	return number(zdi_in(port));
}

object *fn_out(object *args, object *env)
{
	int port = checkinteger(ZDI_OUT, first(args));
	int value = checkinteger(ZDI_OUT, second(args));
	zdi_out(port, value);
	return nil;
}

object *fn_zdi_exec(object *args, object *env)
{
	uint8_t opcodes[5];
	int count;

	opcodes[0] = checkinteger(ZDI_EXEC, first(args));

	for (count = 1, args = cdr(args); count < 5 && args != NULL; count++, args = cdr(args)) {
		opcodes[count] = checkinteger(ZDI_EXEC, first(args));
	}
	zdi_exec(opcodes, count);
	return nil;
}

object *fn_zdi_status(object *args, object *env)
{
	zdi_status();
	return nil;	// TODO return status as an integer?
}

object *fn_zdi_reg(object *args, object *env)
{
	zdi_registers();
	return nil;
}

object *fn_zdi_stk(object *args, object *env)
{
	int depth = (first(args) == NULL) ? 10 : checkinteger(ZDI_STACK, first(args));
	zdi_stacktrace(depth);
	return nil;
}

object *fn_zdi_stop(object *args, object *env)
{
	zdi_stop();
	return nil;
}

object *fn_zdi_run(object *args, object *env)
{
	zdi_run();
	return nil;
}

object *fn_zdi_brk(object *args, object *env)
{
	if (args == NULL) {
		// list breakpoints
		for (int i = 0; i < 4; i++) {
			uint32_t bkpt = zdi_get_break(i);
			if (bkpt <= 0xffffff) {
				printf("#%d: %06lx\r\n", i, bkpt);
			}
		}
	} else {
		int bpnum = checkinteger(ZDI_BRK, first(args));
		if (bpnum < 0 || bpnum > 3) {
			printf("bpnum should be between 0 and 3\r\n");
		} else if (cdr(args) != NULL) {
			zdi_set_break(bpnum, checkinteger(ZDI_BRK, second(args)));
		} else {
			zdi_clear_break(bpnum);
		}
	}
...
object *fn_zdi_step(object *args, object *env)
{
	zdi_stepinto();
	return nil;
}

object *fn_zdi_over(object *args, object *env)
{
	zdi_stepover();
	return nil;
}

object *fn_zdi_until(object *args, object *env)
{
	int address = checkinteger(ZDI_UNTIL, first(args));
	zdi_stepuntil(address);
	return nil;
}

object *fn_zdi_mem(object *args, object *env)
{
	int address = checkinteger(ZDI_MEM, first(args));
	zdi_memdump(address);
	return nil;
}
...
const tbl_entry_t lookup_table[] PROGMEM = {
  ...
  { string212, NULL, 0x00 },
#if defined(CPU_L073)
  { "zdi?", fn_zdi_read, 0x11 },
  { "zdi!", fn_zdi_write, 0x22 },
  { "in", fn_in, 0x11 },
  { "out", fn_out, 0x22 },
  { "status", fn_zdi_status, 0x00 },
  { "regs", fn_zdi_reg, 0x00 },
  { "stack", fn_zdi_stk, 0x01 },
  { "exec", fn_zdi_exec, 0x15 },
  { "stop", fn_zdi_stop, 0x00 },
  { "run", fn_zdi_run, 0x00 },
  { "brk", fn_zdi_brk, 0x02 },
  { "step", fn_zdi_step, 0x00 },
  { "over", fn_zdi_over, 0x00 },
  { "until", fn_zdi_until, 0x11 },
  { "mem", fn_zdi_mem, 0x11 },
  { "ihexload", fn_ihex_load, 0x00 },
  { "", NULL, 0x00 },
#elif defined(CPU_ATSAMD21)
...
void testescape () {
  // TODO support escape? no 'peek' in the CDC rx buffer as yet...
#ifndef consolesupport
  ...
  if (consp(function)) {
    ...
    if (issymbol(car(function), LAMBDA)) {
      ...
      if (trace) {
        ...
        pint(TraceDepth[trace-1], pserial);
        pserial(':'); pserial(' ');
        printobject(fname, pserial); pstring(PSTR(" returned "), pserial);
...
void pfloat (float f, pfun_t pfun) {
  if (isnan(f)) { pstring(PSTR("NaN"), pfun); return; }
  if (f == 0.0) { pfun('0'); return; } // @suppress("Direct float comparison")
  if (isinf(f)) { pstring(PSTR("Inf"), pfun); return; }
  if (f < 0) { pfun('-'); f = -f; }
  // Calculate exponent
  int e = 0;
  if (f < 1e-3 || f >= 1e5) {
...
void pstream (object *form, pfun_t pfun) {
  pfun('<');
  pstring(streamname[(form->integer)>>8], pfun);
  pstring(PSTR("-stream "), pfun);
  pint(form->integer & 0xFF, pfun);
  pfun('>');
}

void printobject (object *form, pfun_t pfun) {
  if (form == NULL) pstring(PSTR("nil"), pfun);
  else if (listp(form) && issymbol(car(form), CLOSURE)) pstring(PSTR("<closure>"), pfun);
  ...
  else if (form->type == CODE) pstring(PSTR("code"), pfun);
...
#ifdef consolesupport
	printf("\e[%c%c%c%c", (char)('0'+p/100), (char)('0'+(p/10)%10), (char)('0'+p%10), c);
#else
...
void setup () {
  ...
  initsleep();
  initgfx();
  pstring(PSTR("uLisp 3.4 "), pserial); pln(pserial);
}
